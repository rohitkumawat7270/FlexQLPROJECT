...
static void exec_ignore_error(FlexQL *db, const string &sql) {
    char *errMsg = nullptr;
    int rc = flexql_exec(db, sql.c_str(), nullptr, nullptr, &errMsg);
    if (errMsg) {
        flexql_free(errMsg);
...
static bool run_data_level_unit_tests(FlexQL *db) {
    ...
    exec_ignore_error(db, "DROP TABLE TEST_ORDERS;");
    exec_ignore_error(db, "DROP TABLE TEST_USERS;");
...
static bool run_insert_benchmark(FlexQL *db, long long target_rows) {
    exec_ignore_error(db, "DROP TABLE BIG_USERS;");
